...
object BuildJT {
  ...
  private def hourlyArrayClosest(
    ...
    val fWithGrid = fBase
      .withColumn(s"${prefix}_targets",
        expr(s"sequence($tsColInF - INTERVAL 12 HOURS, $tsColInF, INTERVAL 1 HOURS)")
      )
    ...
    val partCols: Seq[Column] = fBase.columns.map(col) :+ col(s"${prefix}_target")
    val w = Window
      .partitionBy(partCols: _*)
      .orderBy(abs(col("obs_utc").cast("long") - col(s"${prefix}_target").cast("long")))
  ...
  def buildJT(
    ...
    // ========= ÉTIQUETTE C =========
    val withC = withWd.withColumn("C", when(col("ARR_DELAY_NEW") >= lit(thMinutes), lit(1)).otherwise(lit(0)))
