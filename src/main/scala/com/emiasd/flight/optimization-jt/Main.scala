...
object Main {
  ...
  def runBronze(spark: SparkSession, paths: IOPaths, cfg: AppConfig): Unit = {
    ...
    if (cfg.env == Environment.Local) {
      ...
      logger.info(
        "Env = Hadoop : on saute les CSV d’analyse Bronze pour éviter les problèmes de droits HDFS/local."
      )
    }
  ...
  def runGold(spark: SparkSession, paths: IOPaths, cfg: AppConfig): Unit = {
    ...
    val jtForWrite =
      if (cfg.env == Environment.Hadoop) {
        ...
        logger.info(
          "Env = Local : pas de repartition explicite, on laisse AQE gérer"
        )
        jtSlim
      }
    ...
    val jtForTargets = {
      logger.info(
        "Relecture de JT depuis Delta pour la génération des targets (Local & Hadoop)"
      )
