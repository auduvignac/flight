...
import com.emiasd.flight.analysis.BronzeWeatherCheck

/**
 * Diagnostic tool to identify why weather features have zero importance.
 * 
 * This script performs 4 key analyses:
 * 1. Array population (Wo/Wd emptiness)
 * 2. Struct content quality (NULL fields)
 * 3. Feature variance after extraction
 * 4. Weather source data coverage
 */
object WeatherFeatureDiagnostics {
  ...
  def checkArrayPopulation(df: DataFrame): Unit = {
    ...
    logger.info(s"Empty Wd arrays: $emptyWd / $total (${100.0 * emptyWd / total}%.2f%%)")
  ...
  def checkStructContent(df: DataFrame): Unit = {
    ...
    val weatherFields = Seq(
      "vis", "tempC", "dewC", "rh", "windKt", "windDir",
      "altim", "slp", "stnp", "precip", "wxType", "sky"
    )
    ...
    woNulls.sortBy(-_._2).foreach { case (field, count) =>
      logger.info(f"  o_$field%-15s : $count%8d nulls (${100.0 * count / n}%6.2f%%)")
    }
    ...
    wdNulls.sortBy(-_._2).foreach { case (field, count) =>
      logger.info(f"  d_$field%-15s : $count%8d nulls (${100.0 * count / n}%6.2f%%)")
    }
    ...
    val woCompletelyNull = withArrays
      ...
      .select(
        weatherFields.map(f => 
          col("Wo").getItem(0).getField(s"o_$f").isNull
        ).reduce(_ && _).as("all_null")
      )
      .filter(col("all_null"))
      .count()
    ...
    val wdCompletelyNull = withArrays
      ...
      .select(
        weatherFields.map(f => 
          col("Wd").getItem(0).getField(s"d_$f").isNull
        ).reduce(_ && _).as("all_null")
      )
      .filter(col("all_null"))
      .count()

    logger.info(s"Wo[0] completely NULL: $woCompletelyNull / $n (${100.0 * woCompletelyNull / n}%.2f%%)")
    logger.info(s"Wd[0] completely NULL: $wdCompletelyNull / $n (${100.0 * wdCompletelyNull / n}%.2f%%)")
  ...
  def checkFeatureVariance(
    ...
    val cfg = FeatureBuilder.FeatureConfig(labelCol = "is_pos", testFraction = 0.2, seed = 42L)
    ...
    if (weatherCols.isEmpty) {
      logger.warn("No weather features to analyze (originHours=0 and destHours=0)")
      return
    }
    ...
    logger.info(f"Non-zero variance features: $nonZeroVariance / ${weatherCols.length}")
    ...
    val keyFeatures = Seq("o_vis_avg", "o_temp_avg", "o_wind_avg", "o_precip_sum",
                          "d_vis_avg", "d_temp_avg", "d_wind_avg", "d_precip_sum")
    ...
    if (keyFeatures.nonEmpty) {
      ...
      val stats = withWeather.agg(statsExprs.head, statsExprs.tail: _*).collect()(0)

      keyFeatures.foreach { colName =>
        val meanVal = Option(stats.getAs[Double](s"mean_$colName")).getOrElse(0.0)
        val stdVal = Option(stats.getAs[Double](s"std_$colName")).getOrElse(0.0)
        val minVal = Option(stats.getAs[Double](s"min_$colName")).getOrElse(0.0)
        val maxVal = Option(stats.getAs[Double](s"max_$colName")).getOrElse(0.0)
        
        logger.info(f"  $colName%-20s : mean=$meanVal%8.2f, std=$stdVal%8.2f, min=$minVal%8.2f, max=$maxVal%8.2f")
      }
    }

    // Sample actual feature values
    logger.info("\n--- Sample Feature Values (first 10 rows) ---")
    withWeather.select((Seq("flight_key", "label") ++ keyFeatures).map(col): _*)
  ...
  def checkWeatherSourceCoverage(
    ...
    val flightOrigins = flightsDF.select(col("origin_airport_id")).distinct().count()
    val flightDests = flightsDF.select(col("dest_airport_id")).distinct().count()
    ...
    val inFlightOnly = flightAirportSet.join(weatherAirportSet, Seq("airport_id"), "left_anti").count()
    val inWeatherOnly = weatherAirportSet.join(flightAirportSet, Seq("airport_id"), "left_anti").count()

    logger.info(s"\nAirports in BOTH flights and weather: $inBoth")
    logger.info(s"Airports in flights but NOT in weather: $inFlightOnly (${100.0 * inFlightOnly / flightAirports}%.2f%%)")
    ...
    // Temporal coverage
    logger.info("\n--- Temporal Coverage ---")
    val weatherTemporal = weatherDF.agg(
      min(col("obs_utc")).as("weather_min_ts"),
      max(col("obs_utc")).as("weather_max_ts"),
      count(lit(1)).as("weather_obs_count")
    ).collect()(0)

    val flightTemporal = flightsDF.agg(
      min(col("dep_ts_utc")).as("flight_min_ts"),
      max(col("dep_ts_utc")).as("flight_max_ts"),
      count(lit(1)).as("flight_count")
    ).collect()(0)

    logger.info(s"Weather observations: ${weatherTemporal.getAs[Long]("weather_obs_count")}")
    logger.info(s"  Time range: ${weatherTemporal.getAs[Any]("weather_min_ts")} to ${weatherTemporal.getAs[Any]("weather_max_ts")}")
    
    logger.info(s"\nFlights: ${flightTemporal.getAs[Long]("flight_count")}")
    logger.info(s"  Time range: ${flightTemporal.getAs[Any]("flight_min_ts")} to ${flightTemporal.getAs[Any]("flight_max_ts")}")

    // Observations per airport
    logger.info("\n--- Weather Observations per Airport (sample) ---")
    weatherDF.groupBy(col("airport_id"))
  ...
  def runDiagnostics(
    ...
    logger.info(s"Feature window: originHours=$originHours, destHours=$destHours")

    // Load Gold table
    val targetsPath = goldJTPath.substring(0, goldJTPath.lastIndexOf('/')) + "/targets"
    ...
    checkWeatherSourceCoverage(spark, targetsDF, silverWeatherPath, silverFlightsPath)
