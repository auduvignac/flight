  ...
  def trainAndEvaluate(
    ...
    try {
      ...
      val rfModel = model.stages
        .collect { case m: RandomForestClassificationModel => m }
        .head

      val assembler = model.stages
      ...
      case e: Throwable =>
        logger.warn(s"[ModelingPipeline] Impossible de calculer les importances pour $tag : ${e.getMessage}")
    }
