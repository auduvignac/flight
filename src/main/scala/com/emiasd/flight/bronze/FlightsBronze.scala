  ...
  def readAndEnrich(
    ...
    val f1 = raw
      .withColumn(
        ...
        col("OP_CARRIER_AIRLINE_ID").cast("int")
      )
    ...
    val f3 = f2.withColumn(
      "CRS_DEP_TS_LOCAL",
      parseLocal(col("FL_DATE"), col("CRS_DEP_TIME"))
    )
    ...
    val f4 = f3
      .withColumn(
        "elapsed_min",
        coalesce(col("CRS_ELAPSED_TIME").cast("int"), lit(0))
      )
      .withColumn(
        "dep_ts_utc",
        addMinutes(col("CRS_DEP_TS_LOCAL"), -col("origin_tz") * 60)
      )
      .withColumn(
        "arr_ts_utc",
        addMinutes(col("dep_ts_utc"), col("elapsed_min"))
      )
